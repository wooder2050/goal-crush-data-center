...
const meta: Meta = {
  ...
  parameters: {
    docs: {
      description: {
        component: 'Color palette and tokens used throughout the Goal Crush design system.',
      },
    },
  },
};
...
const ColorGrid = ({ colors: colorObj, title, description }: {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600">
          Grayscale palette for backgrounds, borders, and text. Based on HSL values from our CSS custom properties.
        </p>
      </div>
      <div className="grid gap-2">
        {Object.entries(colors.neutral).map(([key, value]) => (
          <div key={key} className="flex items-center space-x-3 p-2 border rounded">
            <div 
    ...
    <div className="space-y-8">
      {Object.entries(colors.semantic).map(([category, colorScale]) => (
        <div key={category} className="space-y-4">
          <div>
            <h3 className="text-lg font-semibold capitalize">{category} Colors</h3>
            <p className="text-sm text-gray-600">
              {category === 'success' && 'Used for positive actions, confirmations, and success states.'}
              {category === 'warning' && 'Used for warnings, cautions, and attention-grabbing elements.'}
              {category === 'error' && 'Used for errors, destructive actions, and critical alerts.'}
              {category === 'info' && 'Used for informational content and neutral notifications.'}
            </p>
          </div>
          <div className="grid gap-2">
            {Object.entries(colorScale).map(([shade, color]) => (
              <div key={shade} className="flex items-center space-x-3 p-2 border rounded">
                <div 
                  className="w-8 h-8 rounded border shadow-sm flex-shrink-0"
                  style={{ backgroundColor: color }}
                />
                <div className="flex-grow">
                  <span className="font-semibold text-sm">{category}.{shade}</span>
                  <div className="text-xs text-gray-500 font-mono">{color}</div>
                </div>
              </div>
            ))}
