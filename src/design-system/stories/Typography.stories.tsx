...
const meta: Meta = {
  ...
  parameters: {
    docs: {
      description: {
        component: 'Typography scales, weights, and text styles used throughout the Goal Crush design system.',
      },
    },
  },
};
...
const TypographyScale = ({ scale, title, description }: {
    ...
    <div className="space-y-6">
      {Object.entries(scale).map(([key, styles]) => (
        <div key={key} className="border-b pb-4">
          ...
          >
            {key === 'body' && '이것은 본문 텍스트의 예시입니다. Goal Crush에서 사용되는 기본 텍스트 스타일입니다.'}
            {key === 'label' && '라벨 텍스트'}
            {key === 'caption' && '캡션이나 부가 정보를 위한 작은 텍스트'}
            {key === 'link' && <a href="#" className="underline">링크 텍스트 스타일</a>}
            {key.startsWith('h') && `${key.toUpperCase()} 제목 - Goal Crush ${key === 'h1' ? '메인' : key === 'h2' ? '섹션' : '하위'} 제목`}
          </div>
  ...
  render: () => (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-600">Available font sizes in the design system</p>
      </div>
      <div className="space-y-3">
        {Object.entries(typography.fontSize).map(([key, size]) => (
          <div key={key} className="flex items-baseline space-x-4 p-3 border rounded">
  ...
  render: () => (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-600">Available font weights in the design system</p>
      </div>
      <div className="space-y-3">
        {Object.entries(typography.fontWeight).map(([key, weight]) => (
          <div key={key} className="flex items-center space-x-4 p-3 border rounded">
  ...
  render: () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600">Different line height values for optimal readability</p>
      </div>
      <div className="space-y-6">
        {Object.entries(typography.lineHeight).map(([key, lineHeight]) => (
          <div key={key} className="p-4 border rounded">
            ...
            <div 
              ...
              className="text-sm"
            >
              이것은 줄 간격 예시 텍스트입니다. Goal Crush 디자인 시스템에서는 다양한 줄 간격을 사용하여 
              최적의 가독성을 제공합니다. 텍스트의 종류와 용도에 따라 적절한 줄 간격을 선택할 수 있습니다.
              This is example text for line height demonstration. The Goal Crush design system uses
              various line heights to provide optimal readability.
            </div>
          </div>
        ))}
      </div>
      ...
      <div>
        ...
        <div className="space-y-6">
          ...
          <Typography variant="body">
            Body - 본문 텍스트입니다. Goal Crush 플랫폼에서 사용되는 기본 텍스트 스타일입니다.
          </Typography>
