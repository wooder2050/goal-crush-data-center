...
export async function GET(request: NextRequest) {
  try {
    ...
    const sortBy = url.searchParams.get('sort_by') || 'goals_conceded_per_match'; // goals_conceded_per_match, save_percentage, clean_sheets
    ...
    const rankings = Array.from(playerStatsMap.values())
      ...
      .map(stats => {
        return {
          ...
          goals_conceded_per_match: stats.matches_played > 0 
