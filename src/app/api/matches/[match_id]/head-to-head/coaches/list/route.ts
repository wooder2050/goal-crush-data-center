...
export async function GET(
  ...
  try {
    ...
    if (!match || !match.home_coach_id || !match.away_coach_id) {
      return NextResponse.json(
        { error: 'Coaches not found for match' },
        { status: 404 }
      );
    }

    const a = match.home_coach_id;
    const b = match.away_coach_id;
    ...
    return NextResponse.json({
      ...
      items,
      current: {
        home_coach_id: match.home_coach_id,
        away_coach_id: match.away_coach_id,
        home_coach_name: match.home_coach?.name ?? null,
        away_coach_name: match.away_coach?.name ?? null,
      },
    });
  } catch (error) {
