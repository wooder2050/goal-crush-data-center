...
export async function GET(
  ...
  try {
    ...
    const seasonStats = Array.from(seasonStatsMap.values())
      .map(stats => ({
        ...stats,
        goals_conceded_per_match: stats.matches_played > 0 ? (stats.goals_conceded / stats.matches_played).toFixed(2) : '0.00',
        clean_sheet_percentage: stats.matches_played > 0 
    ...
    const careerAverages = {
      goals_conceded_per_match: careerTotals.matches_played > 0 
        ? (careerTotals.goals_conceded / careerTotals.matches_played).toFixed(2) 
        : '0.00',
      clean_sheet_percentage: careerTotals.matches_played > 0 
        ? ((careerTotals.clean_sheets / careerTotals.matches_played) * 100).toFixed(1) 
        : '0.0'
    };

    // 최근 경기들 (최대 10경기)
    const recentMatches = goalkeeperMatches
      .slice(0, 10)
      .map(stat => {
        const isHome = stat.team?.team_id === stat.match?.home_team_id;
        const opponent = isHome ? stat.match?.away_team : stat.match?.home_team;
