...
interface Season2SuperResultsProps {
  className?: string;
}

const Season2SuperResults: React.FC<Season2SuperResultsProps> = ({
  className = '',
}) => {
  const {
    data: matches = [],
    isLoading,
    error,
  } = useGoalQuery(getMatchesBySeasonIdPrisma, [4]);
        ...
        {matches.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              시즌 2 슈퍼리그 경기 데이터가 아직 없습니다
            </h3>
            <p className="text-gray-500">
              시즌 2 슈퍼리그 경기 데이터가 입력되면 여기에 표시됩니다.
            </p>
          </div>
        ) : (
          <div>
...
export default Season2SuperResults;
