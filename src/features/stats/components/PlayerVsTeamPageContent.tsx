...
async function fetchPlayers(searchTerm: string): Promise<Player[]> {
  ...
  const response = await fetch(`/api/players?name=${encodeURIComponent(searchTerm)}&limit=20`);
  if (!response.ok) throw new Error('Failed to fetch players');
  return response.json();
}

async function fetchPlayerVsTeamData(playerId: number, seasonId?: number): Promise<PlayerVsTeamData> {
...
type SortOption = 'attack_points' | 'goals' | 'assists' | 'goals_per_match' | 'assists_per_match' | 'attack_points_per_match';
...
export function PlayerVsTeamPageContent() {
  ...
  } = useGoalQuery(fetchPlayers, [searchTerm], {
    enabled: searchTerm.length > 0,
  });

  // Fetch player vs team data
  const { 
    data: playerVsTeamData, 
    isLoading: isLoadingPlayerVsTeam 
                  ...
                  playersData.map((player) => (
                    <Card 
                      ...
                      className={`cursor-pointer transition-all hover:shadow-md ${
                        selectedPlayer === player.player_id ? 'ring-2 ring-primary' : ''
                      }`}
                    ...
                    >
                      <CardContent className="p-4 flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          {player.team && (
                            <div className="text-sm text-gray-500">{player.team.team_name}</div>
                          )}
                        </div>
                      </CardContent>
                    </Card>
        ...
        {selectedPlayer && (
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                <span className="font-bold text-primary">
                  {isLoadingPlayerVsTeam ? '로딩 중...' : playerVsTeamData?.player_name || '선수'}
                </span>
            ...
            <CardContent>
              ...
              ) : playerVsTeamData?.team_records && playerVsTeamData.team_records.length > 0 ? (
                <>
                  {/* 정렬 옵션 */}
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <Select value={sortBy} onValueChange={(value: SortOption) => setSortBy(value)}>
                      ...
                      <SelectContent>
                        <SelectItem value="attack_points">공격포인트 많은순</SelectItem>
                        <SelectItem value="goals">골 많은순</SelectItem>
                        <SelectItem value="assists">도움 많은순</SelectItem>
                        <SelectItem value="attack_points_per_match">경기당 공격포인트 많은순</SelectItem>
                        <SelectItem value="goals_per_match">경기당 골 많은순</SelectItem>
                        <SelectItem value="assists_per_match">경기당 도움 많은순</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="text-center">경기당 골</TableHead>
                          <TableHead className="text-center">경기당 도움</TableHead>
                          <TableHead className="text-center">경기당 공격P</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {getSortedTeamRecords(playerVsTeamData.team_records).map((record) => (
                          <TableRow key={record.opponent_team_id}>
                            ...
                            <TableCell className="text-center">{record.matches_played}</TableCell>
                    ...
                    {getSortedTeamRecords(playerVsTeamData.team_records).map((record) => (
                      <Card key={record.opponent_team_id} className="p-4">
                        <div className="mb-4">
                          ...
                          <div className="text-sm text-gray-600">{record.matches_played}경기</div>
                        </div>
                        
                        <div className="grid grid-cols-3 gap-4 text-center">
                          <div>
                            <div className="text-2xl font-bold text-blue-600">{record.goals}</div>
                            <div className="text-xs text-gray-500">골</div>
                            <div className="text-xs text-gray-400">{record.goals_per_match}/경기</div>
                          </div>
                          <div>
                            <div className="text-2xl font-bold text-green-600">{record.assists}</div>
                            <div className="text-xs text-gray-500">도움</div>
                            <div className="text-xs text-gray-400">{record.assists_per_match}/경기</div>
                          </div>
                          <div>
                            <div className="text-2xl font-bold text-primary">{record.attack_points}</div>
                            <div className="text-xs text-gray-500">공격P</div>
                            <div className="text-xs text-gray-400">{record.attack_points_per_match}/경기</div>
                          </div>
                        </div>
                      </Card>
                    ))}
                  ...
                  <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                    <h3 className="font-semibold text-lg mb-3">전체 통계 요약</h3>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      <div className="text-center">
                        <div className="text-2xl font-bold text-gray-900">
                          {playerVsTeamData.team_records.reduce((sum, record) => sum + record.matches_played, 0)}
                        </div>
                        <div className="text-sm text-gray-600">총 경기수</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-blue-600">
                          {playerVsTeamData.team_records.reduce((sum, record) => sum + record.goals, 0)}
                        </div>
                        <div className="text-sm text-gray-600">총 골</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-green-600">
                          {playerVsTeamData.team_records.reduce((sum, record) => sum + record.assists, 0)}
                        </div>
                        <div className="text-sm text-gray-600">총 도움</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-primary">
                          {playerVsTeamData.team_records.reduce((sum, record) => sum + record.attack_points, 0)}
                        </div>
                        <div className="text-sm text-gray-600">총 공격포인트</div>
                      </div>
                    </div>
                  </div>
                </>
